...
export const createPagination: CreatePagination = ({
  ...
  const visiblePages = movePages(
    createRange(currentPage - surroundBy, currentPage + surroundBy),
    totalPages
  )
...
const pageExists = (pageNumber: number, totalPages: number): boolean =>
  pageNumber > 0 && pageNumber <= totalPages

const movePageRange = (range: number[], moveBy: number): number[] =>
  range.map((item) => item + moveBy)
...
const movePages = (pages: number[], totalPages: number): number[] => {
  return movePagesLeft(
    movePagesRight(pages, totalPages),
    totalPages
  ).filter((page) => pageExists(page, totalPages))
}
...
const lastItem = <TItem = any>(items: TItem[]): TItem | undefined =>
